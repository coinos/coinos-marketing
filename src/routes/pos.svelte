<div class="w-full lg:w-1/2 px-10 lg:px-0 mx-auto py-10">
	<h1 class="text-4xl mb-2">Point of Sale</h1>
	<h2 class="text-xl mb-10">
		We are excited that you are considering accepting bitcoin at your business and using coinos to
		facilitate this! Please follow the instructions below to get up and running in a matter of
		minutes.
	</h2>

	<ol class="list-decimal space-y-5">
		<li>
			<h3 class="text-xl font-bold mb-2">Register an account</h3>
			<ul class="space-y-5 list-disc">
				<li>
					Visit <a
						href="https://coinos.io"
						rel="noreferrer"
						class="text-primary bg-black p-1 rounded-md">coinos.io</a
					>
					and tap on the
					<strong>REGISTER NEW ACCOUNT</strong>
					button.
					<img src="/register.png" alt="register" class="mt-4 rounded-lg" />
				</li>
				<li>
					Enter a Username and Password into the input fields. <strong>IMPORTANT</strong>
					- remember to write these down so you can access your account again in the future. When ready
					tap on the <strong>REGISTER</strong> button.
					<img src="/register-inputs.png" alt="register-inputs" class="mt-4 rounded-lg" />
				</li>
				<li>
					If you would like to secure your account further, you can tap on the
					<strong>SETUP ACCOUNT</strong> button and set a pin and/or 2FA. Otherwise continue to the next
					step.
				</li>
			</ul>
		</li>
		<li>
			<h3 class="text-xl font-bold mb-2">Receive a payment</h3>
			<ul class="space-y-5 list-disc">
				<li>
					Tap on the <strong>RECEIVE A PAYMENT</strong> button.
					<img src="/receive.png" alt="receive" class="mt-4 rounded-lg" />
				</li>
				<li>
					The
					<strong>LIGHTNING</strong>
					tab is already selected for you which is the preferred method of accepting bitcoin as a PoS
					operator. Tap on the <strong>AMOUNT</strong> button to enter the amount of your sale.
					<img src="/amount.png" alt="amount" class="mt-4 rounded-lg" />
				</li>
				<li>
					You will likely want to select a fiat currency as the unit of account. To do this tap on
					the
					<strong>SAT</strong>
					button and choose a currency from the list, like CAD for example.
					<img src="/units.png" alt="units" class="mt-4 rounded-lg" />
				</li>
				<li>
					Once you have entered the correct amount, tap on the <strong>DONE</strong> button.
					<img src="/enter-amount.png" alt="enter-amount" class="mt-4 rounded-lg" />
				</li>
				<li>
					Now tap on the <strong>SHOW QR</strong> button and turn the display towards the customer.
					They can now scan the QR code with any bitcoin <strong>lightning</strong> wallet on their
					mobile device to make the payment. If they would like to add a tip to the transaction,
					they can tap on one of the pre-determined options or enter a custom amount.
					<img src="/show-qr.png" alt="show-qr" class="mt-4 rounded-lg" />
					<img src="/qr.png" alt="qr" class="mt-4 rounded-lg" />
					<img src="/tip.png" alt="tip" class="my-4 rounded-lg" />
					You can enter this transaction as a cash sale to generate a receipt if the customer asks for
					one.
				</li>
			</ul>
		</li>
		<li>
			<h3 class="text-xl font-bold mb-2">That's it!</h3>
			<p>
				You are now part of the <strong>bitcoin circular economy</strong>, the only thing left to do
				is <strong>HODL</strong> onto those bitcoin. It is recommended for long-term storage to
				withdraw your bitcoin from the coinos platform and put them onto a
				<strong>hardware wallet</strong>. A few options for trusted hardware wallets can be found
				below.
			</p>
			<ul class="list-disc ml-10 mt-4">
				<li>
					<a href="https://www.blockstream.com/jade/" rel="noreferrer" target="_blank"
						>Blockstream Jade</a
					>
				</li>
				<li><a href="https://trezor.io/" rel="noreferrer" target="_blank">Trezor</a></li>
				<li>
					<a href="https://store.coinkite.com/store/coldcard" rel="noreferrer" target="_blank"
						>COLDCARD</a
					>
				</li>
				<li><a href="https://shiftcrypto.ch/" rel="noreferrer" target="_blank">BitBox</a></li>
			</ul>
		</li>
		<li>
			<h3 class="text-xl font-bold mb-2">Customer questions?</h3>
			<p>
				If you have customers asking how they can start buying your products or services with
				bitcoin you can refer them to our <a
					href=""
					rel="noreferrer"
					class="text-primary bg-black p-1 rounded-md">customer guide</a
				>.
			</p>
		</li>
	</ol>
</div>
